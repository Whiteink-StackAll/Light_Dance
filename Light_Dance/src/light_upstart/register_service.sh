...
# 复制脚本到目标路径
chmod +x ./light_clean_up.sh
chmod +x ./light_watch_dog.sh
cp ./light_clean_up.sh /usr/sbin/
cp ./light_watch_dog.sh /usr/sbin/
...
User=$USER_NAME
Type=simple
ExecStart=/usr/sbin/light_watch_dog.sh
ExecStop=/usr/sbin/light_clean_up.sh
Restart=always
TimeoutStopSec=10
RestartSec=10s
